...
static DEVICE_ATTR(size, S_IRWXU, my_chrdev_size_show, my_chrdev_size_store);
    ...
    struct device_attribute *attr, char *buf)
{
    ...
    ret = snprintf(buf, strlen(data->buffer),"%s\n", data->buffer);

	pr_err("%s,%d, %s", __func__, __LINE__, data->buffer);
...
static DEVICE_ATTR(buffer, S_IRWXU, my_chrdev_buffer_show, my_chrdev_buffer_store);
...
static int __init my_chrdev_init(void)
{
    ...
    */
	...
	// 创建属性值, dev是device_create函数返回的struct device,  /sys/class/chrdev0/chrdev0/buffer
    rc = device_create_file(dev, &dev_attr_buffer);
	if (rc) {
		pr_err("%s,%d sys file create failed\n", __func__, __LINE__);
	}
